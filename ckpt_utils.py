...
# 与 DCP 并列保存的整网权重，供 fuse_and_compare / 单机 eval 直接 torch.load（避免 optim 分片缺 __1_0.distcp）
FLAT_MODEL_STATE_FILE = "flat_model_state_dict.pt"
...
def save_checkpoint(
    ...
    # 离线融合/评估：单进程无法可靠加载「model+optimizer」DCP（optim 常落在另一分片 __1_0.distcp）。
    try:
        _flat = {
        ...
        _flat_path = os.path.join(checkpoint_path, FLAT_MODEL_STATE_FILE)
        with fsspec.open(_flat_path, "wb") as f:
            torch.save(_flat, f)
        logger.info("Saved flat model weights for offline eval: %s", _flat_path)
    except Exception as ex:
        logger.warning(
            "Optional %s not saved (fuse_and_compare can fall back to DCP model-only): %s",
